...
class Kereta extends CI_Controller {
    ...
    public function simpantransaksi()
    {
        ...
        $rcpergi = $datapaymentpergi->errorCode;
        $rdpergi = $datapaymentpergi->errorMsg;

        if ($tanggalpulang != '') {

            $datapaymentpulang = $this->paymentkeretapulang();

            $rcpulang = $datapaymentpulang->errorCode;
            $rdpulang = $datapaymentpulang->errorMsg;

            if($rcpergi == '00' && $rcpulang == '00'){

                $tiketpergi = $datapaymentpergi->data->data->url_etiket;
                $strukpergi = $datapaymentpergi->data->data->url_struk;
                $tiketpulang = $datapaymentpulang->data->data->url_etiket;
                $strukpulang = $datapaymentpulang->data->data->url_struk;

                $id= $this->savetotranskeretadetailharga($tiketpergi,$strukpergi,$tiketpulang,$strukpulang);
                ...
                $booking = '<div>'.
                           ...
                           'URL Tiket      : <a href="'.$tiketpergi.'" target="_blank">'.$tiketpergi.'</a><br/>'.
                           'URL Struk      : <a href="'.$strukpergi.'" target="_blank">'.$strukpergi.'</a><br/><br/>'.
                           'Tiket Pulang : <br/>'.
                           'Booking Code   : '.$bookingcodepulang.'<br/>'.
                           'Stasiun Asal   : '.$stfrompulang.'<br/>'.
                           'Stasiun Tujuan : '.$sttopulang.'<br/>'.
                           'Nama Kereta    : '.$trainnamepulang.'<br/>'.
                           'URL Tiket      : <a href="'.$tiketpulang.'" target="_blank">'.$tiketpulang.'</a><br/>'.
                           'URL Struk      : <a href="'.$strukpulang.'" target="_blank">'.$strukpulang.'</a><br/><br/>'.
                           '</div>';

                $this->session->set_flashdata('messageberhasil', $booking);


            }else if($rcpergi == '00' && $rcpulang != '00'){
                //cancel yang pergi
                $datacancel = $this->cancelbookpergi();
                ...
                $this->session->set_flashdata('messagegagalpaymentpulang', $message);

            }else if($rcpergi != '00' && $rcpulang == '00'){
                //cancel yang pulang
                $datacancel = $this->cancelbookpulang();
                ...
                $this->session->set_flashdata('messagegagalpaymentpergi', $message);
                ...
                $this->session->set_flashdata('messagegagalpayment', $message);
            ...
            if($rcpergi == '00'){
                ...
                $booking = '<div>'.
                           ...
                           'Nama Kereta    : '.$trainnamepergi.'<br/>'.
                           'URL Tiket      : <a href="'.$tiketpergi.'" target="_blank">'.$tiketpergi.'</a><br/>'.
                           'URL Struk      : <a href="'.$strukpergi.'" target="_blank">'.$strukpergi.'</a><br/>'.
                           '</div>';
    ...
    //cancel book pergi ketika payment pulang gagal
    public function cancelbookpergi()
    {
        $APIurls = $this->config->item('api2_traincancelbook');

        $txacancel = 'Payment untuk transaksi pulang gagal, sehingga kami mengcancel untuk transaksi pergi';
        $txbcancelcodebookingpergi = $this->input->post('txbbookingcodepergi');
        $txbcanceltransactionidbookingpergi = $this->input->post('txbidtransactionpergi');
    ...
    //cancel book pulang ketika payment pergi gagal
    public function cancelbookpulang()
    {
        $APIurls = $this->config->item('api2_traincancelbook');

        $txacancel = 'Payment untuk transaksi pergi gagal, sehingga kami mengcancel untuk transaksi pulang';
        $txbcancelcodebookingpergi = $this->input->post('txbbookingcodepulang');
        $txbcanceltransactionidbookingpergi = $this->input->post('txbidtransactionpulang');
    ...
    function savetotransaksi($idtranskereta){
        ...
        if ($tanggalpulang != '') {
            ...
            $komisipulang = $this->input->post('txbkomisipulang');
            $komisi = $komisipergi + $komisipulang;
        }else{
            $bookingcode = $bookingcodepergi;
            $komisi = $komisipergi;
        }
    ...
    function savetotranskeretadetailharga($tiketpergi,$strukpergi,$tiketpulang,$strukpulang){
        ...
        //update urletiket dan url struk di table trans_kereta_detail

        $wherepergi = array(
            'bookingCode' => $bookingcodepergi
        );

        $dataetiketstrukpergi = array(
            'urlEtiket' => $tiketpergi,
            'urlStruk' => $strukpergi,
        );

        $this->M_kereta->update_data_transkeretadetail($wherepergi,$dataetiketstrukpergi);
        ...
        if ($tanggalpulang != '') {
            ...
            $dataetiketstrukpulang = array(
                'urlEtiket' => $tiketpulang,
                'urlStruk' => $strukpulang,
            );

            $this->M_kereta->update_data_transkeretadetail($wherepulang,$dataetiketstrukpulang);
